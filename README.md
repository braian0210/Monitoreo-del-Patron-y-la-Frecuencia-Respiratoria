...
La respiración es un proceso fisiológico esencial para la vida que permite el intercambio gaseoso a través de la membrana alveolocapilar, suministrando oxígeno (O₂) a los tejidos para el metabolismo celular y eliminando el dióxido de carbono (CO₂) resultante[9].
Este proceso se fundamenta en un ciclo mecánico compuesto por una fase de inspiración activa y una de espiración, que en condiciones normales de reposo es pasiva [3]
. El sistema respiratorio se divide funcionalmente en los pulmones los cuales son los encargados de la conducción y el intercambio, y la bomba ventilatoria es aquella que esta constituida por los centros respiratorios, los nervios periféricos, la estructura ósea del tórax y los músculos respiratorios[9].

La mecánica pulmonar está regida por propiedades físicas críticas como la elasticidad, que permite al sistema volver a su posición inicial tras la deformación; la viscosidad, relacionada con la fricción interna durante el flujo de aire; y la tensión superficial en la capa alveolar [5]
...
.
Clínicamente, la frecuencia respiratoria  se reconoce como un signo vital de alta sensibilidad para el monitoreo de la estabilidad del paciente[2]
...
<img width="1123" height="747" alt="espectro_profereposo" src="https://github.com/user-attachments/assets/507d0391-bd64-46f6-b1b2-bf149838ffaf" />
 <b>Figura 8.</b> Representación en Frecuencias en Reposo.
</p>
...
<img width="178" height="165" alt="image" src="https://github.com/user-attachments/assets/9c45b3af-c75c-494f-b4ae-c3c91a6dacf9" />
 <b>Figura 9.</b> Representación en Frecuencias Hablando.
</p>
...
#Conclusiones 
- Se lograron  medir los ciclos respiratorios mediante la presión ejercida por la deformación torácica, lo que permitió comprender con mayor profundidad las variables involucradas y controlables mediante este método, así como la importancia fisiológica y anatómica durante su implementación. La correcta ubicación del sensor resulta un factor clave para la adquisición satisfactoria de la señal, ya que el sistema es altamente sensible a la posición en la que se aplica la presión y a la adaptabilidad de sus implementos. Debido a la gran variabilidad en las características corporales de cada individuo, se concluye que este método depende en gran medida de una adecuada personalización para la medición de los patrones respiratorios, lo que puede dificultar su aplicación en un ámbito médico general.
- A partir de esta práctica de laboratorio en donde el objetivo era evaluar la influencia que tenia el habla sobre el patrón respiratorio y el estudio de esta señal respiratoria en dos estados, se determina que el procesamiento de la señal respiratoria permitió analizar esta señal tanto en el dominio del tiempo como el de la frecuencia. En estado de reposo, visualmente se observaron 6 ciclos respiratorios en una toma de 30 segundos los cuales corresponden a 12 respiraciones por minuto, valor que fue confirmado mediante el análisis espectral al identificar un pico dominante de 2 Hz. Esto valida el funcionamiento del sistema de adquisición implementado, evidenciando que la frecuencia obtenida está dentro del rango fisiológico esperado.
- Por otra parte durante la condición de habla, se observo una frecuencia respiratoria de 10 respiraciones por minuto, correspondiente a un pico dominante de 0,166667 Hz. Aunque en el dominio del tiempo se identificaron menos ciclos de los esperados, la presencia de la segunda componente espectral cercana a 0,3 Hz evidencia que el hecho de estar hablando, modifica el patrón respiratorio. Esto confirma que el análisis en frecuencia complementa la observación temporal y permite ver cambios que no son visualmente posibles.
